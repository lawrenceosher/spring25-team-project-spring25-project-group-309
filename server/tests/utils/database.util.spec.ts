...
describe('populateDocument', () => {
  ...
  describe('populateTask', () => {
    ...
    it('should fetch and populate a task document', async () => {
      // First findOne returns the base task
      (TaskModel.findOne as jest.Mock).mockResolvedValueOnce(mockTask);

      // Second findOne returns the populated task
      const populatedChain = {
        populate: jest.fn().mockReturnThis(),
        then: (cb: (value: typeof mockTask) => void) => cb(mockTask), // resolves to mockPopulatedTask
      };

      (TaskModel.findOne as jest.Mock).mockReturnValueOnce(populatedChain);

      const result = await populateDocument('taskId', 'task');

      expect(TaskModel.findOne).toHaveBeenCalledWith({ _id: 'taskId' });
      expect(result).toEqual(mockTask);
    });

    it('should return an error if task document is not found', async () => {
      (TaskModel.findOne as jest.Mock).mockResolvedValueOnce(null);

      const result = await populateDocument('invalidTaskId', 'task');

      expect(result).toEqual({
        error: 'Error when fetching and populating a document: Task not found',
      });
    });

    it('should return an error if populated task document is not found', async () => {
      // First findOne returns a valid task
      (TaskModel.findOne as jest.Mock).mockResolvedValueOnce(mockTask);

      // Second findOne (populated task) returns null
      (TaskModel.findOne as jest.Mock).mockReturnValueOnce({
      ...
      expect(result).toMatchObject({
        error: expect.stringContaining('Error when fetching and populating a document:'),
      });
    });

    it('should return an error message if fetching a task document throws an error', async () => {
